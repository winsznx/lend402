              ...
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center gap-1.5 shrink-0 min-w-0 px-1">
                  <div
                    ...
                    style={{
                      background: isActive
                        ...
                        : undefined,
                      border: `2px solid ${
                        isActive ? "#22d3ee" : isCompleted ? "#4ade80" : "rgba(226,232,240,0.15)"
                      }`,
                  ...
                  <p
                    ...
                    style={{ fontSize: "9px", letterSpacing: "0.06em", color: isActive ? "#22d3ee" : isCompleted ? "#4ade80" : undefined }}
                ...
                {i < steps.length - 1 && (
                  <div
                    className={`flex-1 h-px transition-all duration-700 min-w-[12px] ${!isCompleted ? "bg-slate-200 dark:bg-slate-800" : ""}`}
                    style={isCompleted ? { background: "linear-gradient(90deg, #4ade8060, #4ade8030)" } : undefined}
                  />
                )}
              </React.Fragment>
            );
  ...
  return (
    <GlassCard accentColor="#f87171" className="border-red-300/40 dark:border-red-500/30">
      <div className="px-4 py-3 flex items-start gap-3">
        ...
        <div>
          <p className="font-mono text-xs font-bold text-red-600 dark:text-red-400 mb-0.5">AGENT ERROR</p>
          <p className="font-mono text-xs text-red-500/80 dark:text-red-500/70">{state.lastError}</p>
        </div>
      </div>
    </GlassCard>
  );
      ...
      <div className="px-4 py-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
        {refs.map((r) => (
          <div key={r.label} className="flex flex-col gap-0.5">
            <span className="font-mono text-[9px] tracking-widest text-slate-400 dark:text-slate-600 uppercase">{r.label}</span>
...
function CommandCenterDashboard() {
  ...
  return (
    ...
    >
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-slate-200/80 dark:border-slate-800/60 bg-white/40 dark:bg-slate-950/60 backdrop-blur-sm">
        <div className="max-w-screen-xl mx-auto px-4 md:px-6 h-11 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link href="/docs" className="font-mono text-[9px] tracking-widest text-slate-400 dark:text-slate-700 hover:text-slate-600 dark:hover:text-slate-500 transition-colors uppercase">
              Docs
            </Link>
            <a href="https://docs.stacks.co" target="_blank" rel="noopener noreferrer" className="font-mono text-[9px] tracking-widest text-slate-400 dark:text-slate-700 hover:text-slate-600 dark:hover:text-slate-500 transition-colors uppercase">
              Stacks Docs
            </a>
            <a href="https://explorer.hiro.so" target="_blank" rel="noopener noreferrer" className="font-mono text-[9px] tracking-widest text-slate-400 dark:text-slate-700 hover:text-slate-600 dark:hover:text-slate-500 transition-colors uppercase">
              Explorer
            </a>
          </div>
        </div>
...
// ---------------------------------------------------------------------------
// ROOT — AgentProvider + ThemeProvider live in the (app) layout above
// ---------------------------------------------------------------------------
